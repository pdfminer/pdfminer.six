...
def debug_storage(storage, base_info=False, chars=True, runs=False):
    ...
    if chars:
        ...
        for _ch in storage['chars']:
            if _ch != '\n':
                print(f"output so far: {output} type {type(output)}")
                print(f"ch: {_ch} type: {type(_ch)}")
                print(f"ch['ch']: {_ch['ch']} type: {type(_ch['ch'])}")

                formattedCh = _ch['ch'].get_text()
                # print(f"string: {string} type: {type(string)}")
                # output += _ch['ch']
                output = output + formattedCh
            else:
                output += 'C'
